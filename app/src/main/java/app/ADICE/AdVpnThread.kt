...
/**
 * Retrieves the first available DNS server for the current network.
 * @param context Application context
 * @return The first DNS server address
 * @throws VpnNetworkException if no DNS server is found
 */
...
/**
 * Thread responsible for running the VPN logic, handling DNS requests, and blocking hosts.
 * @param vpnService The VpnService instance
 * @param notify Callback to notify status changes
 */
class AdVpnThread(
    private var vpnService: VpnService,
    private var notify: ((Int) -> Unit)?
) : Runnable {
    companion object {
    ...
    /**
     * Starts the VPN thread.
     */
    fun startThread() {
        Log.i(TAG, "Starting Vpn Thread")
        thread = Thread(this, "ADICEVpnThread").apply { start() }
    ...
    /**
     * Main run loop for the VPN thread.
     */
    @Synchronized
    override fun run() {
        try {
            ...
            // Load the block list
            loadBlockedHosts()
            notify?.invoke(VPN_STATUS_STARTING)
            ...
            while (true) {
                try {
                    ...
                    // We want to filter out VpnNetworkException from our crash analytics as these
                    ...
                    notify?.invoke(VPN_STATUS_RECONNECTING_NETWORK_ERROR)
                } catch (e: Exception) {
            ...
            notify?.invoke(VPN_STATUS_STOPPING)
            Log.i(TAG, "Exiting")
        }
    }

    /**
     * Runs the VPN, handling packets and DNS requests.
     * @throws Exception if VPN setup or packet handling fails
     */
    ...
    private fun runVpn() {
        ...
        try {
            ...
            notify?.invoke(VPN_STATUS_RUNNING)
            ...
            while (true) {
                ...
                Log.i(TAG, "Starting new thread to handle dns request (active = ${executor.activeCount} backlog = ${executor.queue.size})")
                ...
                try {
                    ...
                    throw VpnNetworkException("High backlog in dns thread pool executor, network probably stalled")
                }
            }
        } finally {
    ...
    /**
     * Handles a single DNS request, blocking or forwarding as needed.
     * @param packet The raw packet data
     * @param dnsSocket The socket to use for DNS
     * @param outFd Output stream for the VPN
     */
    private fun handleDnsRequest(packet: ByteArray, dnsSocket: DatagramSocket, outFd: FileOutputStream) {
        try {
            ...
            if (!blockedHosts.orEmpty().contains(dnsQueryName)) {
    ...
    /**
     * Loads the blocked hosts from asset files.
     */
     ...
     * @return ParcelFileDescriptor for the VPN interface
